"""
Sports Tracker Web App
======================
Flask + SocketIO backend. Serves the UI and runs live sport tracking
in background threads, pushing updates to the browser via WebSocket
and to iPhone via ntfy.sh.

Install:
    pip install flask flask-socketio requests MLB-StatsAPI eventlet
...
Deploy on Render:
    ...
    - Set env vars:   NTFY_TOPIC, SECRET_KEY
"""
...
NTFY_TOPIC    = os.environ.get("NTFY_TOPIC", "my-sports-alerts-change-me")
...
# One active tracker thread at a time
tracker_thread = None
...
def log(msg: str):
    """Emit a log line to the browser and print to console."""
    ts = datetime.now().strftime("%H:%M:%S")
    line = f"[{ts}] {msg}"
    print(line)
    socketio.emit("log", {"text": line})


def alert(player: str, event: str, detail: str, sport: str):
    """Emit a highlighted alert card to the browser."""
    socketio.emit("alert", {
        "player": player,
        "event": event,
        "detail": detail,
...
# ─────────────────────────────────────────────────────────────
#  ROUTES — DATA APIs (called by the browser UI)
# ─────────────────────────────────────────────────────────────
...
def api_games(sport):
    ...
    if sport == "mlb":
        try:
            ...
            for g in games_raw:
                k = sk(g)
                away, home = g.get("away_name","Away"), g.get("home_name","Home")
                ...
                games.append({"id": g["game_id"], "label": f"{away} @ {home}", "badge": badge, "live": k==0})
        ...
        return jsonify([{"id": e["id"], "label": e.get("name","Event"), "badge": "", "live": True} for e in events])
        ...
        try:
            ...
            for side in ("away", "home"):
                for _, pd in boxscore.get(side,{}).get("players",{}).items():
                    ...
                    if n and pid:
                        players[str(pid)] = {"name": n, "pos": pos}
...
def track_mlb(game_id, player_ids, player_names):
    import statsapi
    seen = set()
    log(f"MLB tracker started — watching {', '.join(player_names.values())}")
    while not tracker_stop.is_set():
        try:
            ...
            for play in plays:
                ...
                log(f"{name}: {event}")
                alert(name, event, detail, "MLB")
                send_notification(f"MLB - {name}: {event}", detail, "high")
                new += 1

            log(f"{len(plays)} plays checked, {new} new alerts")
        except Exception as e:
            log(f"Error: {e}")
        tracker_stop.wait(POLL_INTERVAL)
    log("MLB tracker stopped.")


def track_espn(sport, league, sport_label, game_id, title, player_ids, player_names):
    seen = set()
    log(f"{sport_label} tracker started — watching {', '.join(player_names.values())}")
    while not tracker_stop.is_set():
        try:
            summary = espn_get(f"{ESPN_WEB}/{sport}/{league}/summary", {"event": game_id})
            if espn_is_final(summary):
                score = espn_score_str(summary)
                log(f"Game ended. Final: {score}")
                send_notification(f"{sport_label} Final: {title}", score, "low")
                break
            ...
            for play in plays:
                ...
                if sport_label == "NBA":   time_str = f"Q{period} {clock}"
                elif sport_label == "NHL": time_str = f"P{period} {clock}"
                else:                      time_str = ""
                ...
                for pid in matching:
                    ...
                    log(f"{name}: {text[:60]}")
                    alert(name, play_type, detail, sport_label)
                    send_notification(f"{sport_label} - {name}: {play_type}", detail, "high")
                    new += 1
            ...
            log(f"Error: {e}")
        tracker_stop.wait(POLL_INTERVAL)
    log(f"{sport_label} tracker stopped.")


def track_pga(event_id, event_name, player_ids, player_names):
    last_snapshot = {}
    log(f"PGA tracker started — watching {', '.join(player_names.values())}")
    while not tracker_stop.is_set():
        try:
            ...
            for c in competitors:
                ...
                log(f"{name}: Score={score} Thru={thru}")
                alert(name, f"Score: {score}", detail, "PGA")
                send_notification(f"PGA - {name}: {score}", detail, "high")
                new += 1
            ...
            log(f"{len(competitors)} players checked, {new} score updates")
        except Exception as e:
            log(f"Error: {e}")
        tracker_stop.wait(POLL_INTERVAL)
    log("PGA tracker stopped.")
...
def handle_start(data):
    global tracker_thread, tracker_stop
    ...
    send_notification(f"Tracking {game_label}", f"Watching: {', '.join(players.values())}", "low")
    log(f"Starting {sport.upper()} tracker for {game_label}")

    if sport == "mlb":
        pid_map = {int(k): v for k, v in players.items()}
        tracker_thread = threading.Thread(
            target=track_mlb,
            args=(int(game_id), set(pid_map.keys()), pid_map),
            daemon=True)
        ...
        tracker_thread = threading.Thread(
            target=track_espn,
            args=(sp, lg, label, game_id, game_label, set(players.keys()), players),
            daemon=True)

    elif sport == "pga":
        tracker_thread = threading.Thread(
            target=track_pga,
            args=(game_id, game_label, set(players.keys()), players),
            daemon=True)
...
<div class="container">
  ...
  <!-- STEP 1: SPORT -->
  <div class="step" id="step-sport">
    <div class="step-label">Step 1 — Select Sport</div>
    <div class="btn-grid">
      <button onclick="selectSport('mlb')">⚾ MLB</button>
      <button onclick="selectSport('nba')">🏀 NBA</button>
      <button onclick="selectSport('nhl')">🏒 NHL</button>
      <button onclick="selectSport('pga')">⛳ PGA Tour</button>
    </div>
  </div>

  <!-- STEP 2: GAME -->
  <div class="step disabled" id="step-game">
    <div class="step-label">Step 2 — Select Game / Event</div>
  ...
  <div class="step disabled" id="step-players">
    <div class="step-label">Step 3 — Select Players</div>
  ...
  <div class="step disabled" id="step-start">
    <div class="step-label">Step 4 — Start Tracking</div>
...
let selectedGame   = null;   // {id, label}
let selectedPlayers = {};    // {id: name}
let allPlayers     = {};     // {id: {name, pos}} full roster
...
socket.on("tracking_started", data => {
  ...
  appendLog(`Tracking started: ${data.game} | Players: ${data.players.join(", ")}`);
});
...
function selectSport(sport) {
  ...
  // Reset downstream steps — players and start stay disabled until game is picked
  setStep("step-game", false);
  setStep("step-players", true);   // disabled until game selected
  setStep("step-start", true);     // disabled until players selected
  document.getElementById("games-list").innerHTML = '<div class="empty-state"><span class="loader"></span> Loading games...</div>';
  ...
  fetch(`/api/games/${sport}`)
    .then(r => r.json())
    .then(games => {
      setStep("step-game", false);  // enable
  ...
  fetch(`/api/players/${selectedSport}/${game.id}`)
    ...
    .then(players => {
      if (players.error || !Object.keys(players).length) {
        ...
        setStep("step-players", false);  // still enable so user can see the message
        return;
      }
      allPlayers = players;
      setStep("step-players", false);  // ← FIX: enable Step 3 once players are loaded
